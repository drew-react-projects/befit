 ...
 */
class Weightsliderscrollbg extends React.PureComponent {
  render() {
    return (
      <Image
...
export default class Weight extends React.Component {
  state = {
    weight_slider_value: 0, //weight slider value
    weight_slider_items: Array.from(Array(250).keys()), //maximum number of the weight slider(250)
  };
      ...
      <View style={styles.weightsliderbox}>
        ...
        <View style={styles.weightsliderheader}>
          ...
          <Text style={styles.weightsliderh1}>
            {this.state.weight_slider_value < 5
              ? "-"
              : this.state.weight_slider_value - 5}
          </Text>
          {/*weight slider value*/}
          <Text style={styles.weightsliderh2}>
            {this.state.weight_slider_value}
          </Text>
          {/*weight slider value +5 */}
          <Text style={styles.weightsliderh3}>
            {this.state.weight_slider_value <
            this.state.weight_slider_items.length
              ? this.state.weight_slider_value + 5
              : "-"}
          </Text>
        </View>
        ...
        <View style={styles.weightslider}>
          ...
          <FlatList
            ...
            data={this.state.weight_slider_items}
            horizontal={true}
            decelerationRate={"fast"}
            keyExtractor={(item, index) => index.toString()}
            showsHorizontalScrollIndicator={false}
            onScroll={(event) => {
              this.weight_slider_onscroll(event);
            }}
  ...
  weight_slider_onscroll(event) {
    ...
    this.setState({
      weight_slider_value: Math.round(event.nativeEvent.contentOffset.x / 32),
    });
  }
  render() {
